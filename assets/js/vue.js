// data
const products = [
  {
    id: 1,
    description: "Quarz Luxe",
    price: 12,
    img: "assets/img/quarz-luxe.JPG",
  },
  {
    id: 2,
    description: "Curren Business",
    price: 20,
    img: "assets/img/curren-business.JPG",
  },
  {
    id: 3,
    description: "Curren Sport",
    price: 5,
    img: "assets/img/curren-sport.JPG",
  },
  {
    id: 4,
    description: "Jaragar Racing",
    price: 8,
    img: "assets/img/jaragar-racing.JPG",
  },
  {
    id: 5,
    description: "Liges Hommes",
    price: 3,
    img: "assets/img/liges-hommes.JPG",
  },
  {
    id: 6,
    description: "Maserati Mechanical",
    price: 65,
    img: "assets/img/maserati-mechanical.JPG",
  },
  {
    id: 7,
    description: "Montre Mecanique",
    price: 25,
    img: "assets/img/montre-mecanique.JPG",
  },
  {
    id: 8,
    description: "Brand Designer",
    price: 28,
    img: "assets/img/brand-designer.JPG",
  },
  {
    id: 9,
    description: "Relogio Masculino",
    price: 4,
    img: "assets/img/relogio-masculino.JPG",
  },
  {
    id: 10,
    description: "Tissot Multifunction",
    price: 29,
    img: "assets/img/tissot-multifunction.JPG",
  },
  {
    id: 11,
    description: "Hip Hop Gold",
    price: 87,
    img: "assets/img/hiphop-gold.JPG",
  },
  {
    id: 12,
    description: "Mesh Genova",
    price: 6,
    img: "assets/img/mesh-genova.JPG",
  },
];

const Home = {
  template: "#home",
  name: "Home",
  data: () => {
    return {
      products,
      searchKey: "",
      liked: [],
      cart: [],
      total: 0,
      totalArticle: 0,
    };
  },
  computed: {
    // Ici vue surveille le dom en permanence
    filteredList() {
      //on récupère les éléments filtrés
      return this.products.filter((product) => {
        return product.description
          .toLowerCase()
          .includes(this.searchKey.toLowerCase());
      });
    },
    // On récupère les éléments likés enregistrés en cookie
    getLikeCookie() {
      console.log("lancement fonction getLikeCookie");
      let cookieValue = JSON.parse($cookies.get("like"));
      cookieValue == null ? (this.liked = []) : (this.liked = cookieValue);
      // Affichage d'un point sur le coeur de la nav lorsque des éléments sont enregistrés en favoris
      let notif = document.getElementById("nav-not");
      if (this.liked.length >= 1) {
        notif.style.visibility = "visible";
        notif.style.opacity = "1";
      } else {
        notif.style.visibility = "hidden";
        notif.style.opacity = "0";
      }
    },
    cartTotalAmount() {
      let total = 0;
      for (let item in this.cart) {
        total = total + this.cart[item].quantity * this.cart[item].price;
      }
      return total;
    },
    cartTotalArticle() {
      let totalArticle = 0;
      for (let item in this.cart) {
        totalArticle = totalArticle + this.cart[item].quantity;
      }
      return totalArticle;
    },
  },
  methods: {
    // les méthodes ici ne seront appelé qu'en cas d'un événement (un click par exemple)
    setLikeCookie() {
      // mets à jour les cookies en fonction du tableau liked
      document.addEventListener("input", () => {
        setTimeout(() => {
          $cookies.set("like", JSON.stringify(this.liked));
        }, 300);
      });
    },
    setNotif() {
      let checkbox = document.getElementsByClassName("checkFav");
      console.log(checkbox);
    },
    addToCart(product) {
      for (let i = 0; i < this.cart.length; i++) {
        if (this.cart[i].id === product.id) {
          // this.total = this.total + this.cart[i].price;
          // this.totalArticle = this.totalArticle + 1;
          return this.cart[i].quantity++;
        }
      }
      this.cart.push({
        id: product.id,
        img: product.img,
        description: product.description,
        price: product.price,
        quantity: 1,
      });
      // this.total = this.total + product.price;
      // this.totalArticle = this.totalArticle + 1;
    },
    addQty(product) {
      // Ajout d'une quantité dans le panier
      console.log(product);
      product.quantity = product.quantity + 1;
      // this.total = this.total + product.price;
      // this.totalArticle = this.totalArticle + 1;
    },
    trash(id, product) {
      // Suppression d'un article du panier
      this.$delete(this.cart, id);
      // this.total = this.total - product.quantity * product.price;
      // this.totalArticle = this.totalArticle - product.quantity;
    },
    reduceQty(product, id) {
      // réduction de la quantité d'un produit dans le panier
      if (product.quantity == 1) {
        this.trash(id);
      } else {
        product.quantity = product.quantity - 1;
        // this.total = this.total - product.price;
        // this.totalArticle = this.totalArticle - 1;
      }
    },
    // addQty(id) {
    //   // augmenter quantité du produit cliqué dans le panier
    //   for (let i = 0; i < this.cart.length; i++) {
    //     if (this.cart[i].id === id) {
    //       return this.cart[i].quantity++;
    //     }
    //   }
    // },
    // reduceQty(id) {
    //   // réduit quantité du produit cliqué dans le panier
    //   for (let i = 0; i < this.cart.length; i++) {
    //     if (this.cart[i].id === id) {
    //       if (this.cart[i].quantity <= 1) {
    //         this.cart.splice(i, 1);
    //       } else {
    //         return this.cart[i].quantity--;
    //       }
    //     }
    //   }
    // },
    // trash(id) {
    //   // Supprime le produit du panier
    //   for (let i = 0; i < this.cart.length; i++) {
    //     if (this.cart[i].id === id) {
    //       return this.cart.splice(i, 1);
    //     }
    //   }
    // },
  },

  mounted() {
    // récupération des cookie au chargement de la page
    this.getLikeCookie;

    // console.log("lancement fonction getLikeCookie");
    // let cookieValue = JSON.parse($cookies.get("like"));
    // cookieValue == null ? (this.liked = []) : (this.liked = cookieValue);
    // // Affichage d'un point sur le coeur de la nav lorsque des éléments sont enregistrés en favoris
    // let notif = document.getElementById("nav-not");
    // if (this.liked.length >= 1) {
    //   notif.style.visibility = "visible";
    //   notif.style.opacity = "1";
    // } else {
    //   notif.style.visibility = "hidden";
    //   notif.style.opacity = "0";
    // }
  },
};
const UserSettings = {
  template: "<h1>UserSettings</h1>",
  name: "UserSettings",
};
const WishList = {
  template: "<h1>WishList</h1>",
  name: "WishList",
};
const ShoppingCart = {
  template: "<h1>ShoppingCart</h1>",
  name: "ShoppingCart",
};

// router
const router = new VueRouter({
  routes: [
    { path: "/", component: Home, name: "Home" },
    { path: "/user-settings", component: UserSettings, name: "UserSettings" },
    { path: "/wish-list", component: WishList, name: "WishList" },
    { path: "/shopping-cart", component: ShoppingCart, name: "ShoppingCart" },
  ],
});

const vue = new Vue({
  router,
}).$mount("#app"); //on pointe ce qu'on veut monter

//on peut faire un montage à part, avec vue.$mount('#app)
